...
func (sm *SyncManager) SyncDataGraphQL(ctx context.Context, dbs []int64) (*table.LSTable, error) {
	...
	for _, db := range dbs {
		...
		if db == 1 {
			...
			// Relay fork: the initial GraphQL sync of secondary databases (2 = contacts,
			// 95 = contact presence) returns real data rows, but upstream kept only db 1's
			// table — so presence/contacts were silently dropped at startup. Dispatch them
			// to the event handler so the app gets an initial online/last-seen snapshot.
			sm.client.HandleEvent(ctx, &Event_PublishResponse{Table: lsTable})
		}
